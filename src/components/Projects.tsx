          ...
          <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-5">
              <div className="rounded-2xl p-6 bg-[#070a14] border-2 border-amber-500/40 shadow-xl relative overflow-hidden">
                ...
                <div className="flex items-center gap-1.5 mb-4 p-1 rounded-xl bg-black/60 border border-white/10 font-mono text-[10px]">
                  {(['AUTO_WAYPOINT', 'OBSTACLE_AVOID', 'SOIL_SAMPLING'] as const).map((mode) => (
                    <button
                      key={mode}
                      type="button"
                      onClick={() => setRoverNavState(mode)}
                      className={`flex-1 py-1 px-1 rounded-lg transition font-bold text-center ${
                        roverNavState === mode
