...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 transition-all duration-300 pt-3 sm:pt-4 px-4 sm:px-6">
      <div className="max-w-6xl mx-auto">
        <nav
          className={`flex items-center justify-between h-14 sm:h-16 px-4 sm:px-6 rounded-2xl transition-all duration-300 ${
            isScrolled
              ? 'bg-[#0a0e1a]/85 backdrop-blur-xl border border-white/[0.12] shadow-[0_12px_40px_rgba(0,0,0,0.6)]'
              : 'bg-[#0a0e1a]/60 backdrop-blur-lg border border-white/[0.06]'
          }`}
        >
          {/* Logo / Brand */}
          <a
          ...
          >
            ...
            <div className="flex flex-col">
              <span className="font-heading font-extrabold tracking-tight text-sm sm:text-base text-white group-hover:text-indigo-300 transition-colors">
                Satya Eesa
              </span>
            ...
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  {link.name}
                  {isActive && (
                    <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-3 h-0.5 rounded-full bg-gradient-to-r from-indigo-400 to-cyan-400" />
                  )}
                </a>
              );
            })}
          ...
          <div className="hidden sm:flex items-center gap-2.5">
            <a
              ...
              className="p-2 rounded-xl text-slate-400 hover:text-white hover:bg-white/[0.06] transition border border-transparent hover:border-white/[0.08]"
            >
            ...
            <a
              ...
              className="p-2 rounded-xl text-slate-400 hover:text-sky-400 hover:bg-white/[0.06] transition border border-transparent hover:border-white/[0.08]"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="group relative inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold text-white bg-gradient-to-r from-indigo-600 via-indigo-500 to-cyan-600 hover:shadow-[0_0_24px_rgba(99,102,241,0.4)] transition-all duration-300"
            >
          ...
          <div className="flex sm:hidden items-center gap-2">
            ...
            <button
              ...
              className="p-2 rounded-xl text-slate-300 hover:text-white hover:bg-white/[0.08] transition"
            >
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-x-4 top-20 z-50 p-5 rounded-2xl bg-[#0b0f1a]/95 backdrop-blur-2xl border border-white/[0.12] shadow-2xl animate-in fade-in slide-in-from-top-4 duration-200">
          <div className="flex flex-col gap-1.5">
            {navLinks.map((link) => (
              <a
                ...
                className="flex items-center justify-between px-4 py-2.5 text-sm font-medium rounded-xl text-slate-200 hover:text-white hover:bg-white/[0.06] transition"
              >
                <span>{link.name}</span>
