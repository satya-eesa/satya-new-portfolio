...
export default function Skills() {
  ...
  return (
    <section id="skills" className="relative py-20 sm:py-32 bg-[#070a12]/90 overflow-hidden">
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex flex-col items-center text-center mb-16">
          ...
          <h2 className="font-heading font-extrabold text-3xl sm:text-5xl text-white tracking-tight">
            Specialized in <span className="bg-gradient-to-r from-indigo-400 via-sky-300 to-cyan-300 bg-clip-text text-transparent">Digital Hardware</span> &amp; Systems
          </h2>
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {category.skills.map((skill) => (
                  <div
                    key={skill.name}
                    className="p-3 rounded-2xl bg-white/[0.02] border border-white/[0.05] hover:border-indigo-500/30 hover:bg-white/[0.05] transition-all flex items-center justify-between"
                  >
                    <span className="font-medium text-xs text-white">
                      {skill.name}
                    </span>
                    {skill.level && (
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-indigo-500/15 text-indigo-300 font-semibold">
                        {skill.level}
                      </span>
                    )}
                  </div>
        ...
        <div className="rounded-3xl glass-panel p-6 sm:p-10 border border-white/[0.08] shadow-xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5">
            {[
              ...
              <div
                key={p.step}
                className="relative p-4 rounded-2xl bg-white/[0.02] border border-white/[0.06] hover:border-cyan-400/40 transition flex flex-col justify-between"
              >
                <div>
                  <span className="text-xs font-mono font-bold text-indigo-400">{p.step}</span>
                  <h5 className="font-heading font-bold text-sm text-white mt-1">{p.title}</h5>
                  <p className="mt-1.5 text-xs text-slate-400 leading-relaxed">{p.desc}</p>
                </div>
                {idx < 4 && (
                  <div className="hidden lg:block absolute -right-2.5 top-1/2 -translate-y-1/2 z-10 text-slate-500 font-mono text-xs">
                    &rarr;
                  </div>
                )}
              </div>
